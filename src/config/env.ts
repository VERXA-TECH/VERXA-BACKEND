...
const envConfig = {
    ...
    device: {
        ...
        trustTokenSecret: (() => {
            const v = requireEnv("DEVICE_TRUST_TOKEN_SECRET", process.env.DEVICE_TRUST_TOKEN_SECRET);
            if (v.length < 32) {
                console.error(`Environment variable DEVICE_TRUST_TOKEN_SECRET must be at least 32 chars`);
                process.exit(1);
            }
            return v;
        })(),
    ...
    email: {
        ...
        from: process.env.SMTP_FROM || "me@verxa.com",
